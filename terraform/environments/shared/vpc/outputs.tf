output "vpc_id" {
  value = module.vpc.vpc_id
}

output "vpc_cidr" {
  value = var.cidr
}
output "public_subnets" {
  value = module.vpc.public_subnets
}

output "private_subnets" {
  value = module.vpc.private_subnets
}

output "public_cidrs" {
  value = module.vpc.public_cidrs
}

output "private_cidrs" {
  value = module.vpc.private_cidrs
}

output "private_route_table_ids" {
  value = module.vpc.private_route_table_ids
}

output "internal_dns_name" {
  value = module.vpc.internal_dns_name
}

output "internal_dns_zone_id" {
  value = module.vpc.internal_dns_zone_id
}

output "external_dns_name" {
  value = module.vpc.external_dns_name
}

output "external_dns_zone_id" {
  value = module.vpc.external_dns_zone_id
}

output "nat_public_ips" {
  value = module.vpc.nat_public_ips
}

output "bastion_keypair_name" {
  value = module.vpc.bastion_keypair_name
}

#output "bastion_private_key_pem" {
#  value = var.create_bastion_keypair ? module.vpc.bastion_private_key_pem : ""
#}

output "bastion_public_key_pem" {
  value = var.create_bastion_keypair ? module.vpc.bastion_public_key_pem : ""
}

output "bastion_public_key_openssh" {
  value = var.create_bastion_keypair ? module.vpc.bastion_public_key_openssh : ""
}

output "bastion_ssm_private_key_pem_arn" {
  value = var.create_bastion_keypair ? module.vpc.bastion_ssm_private_key_pem_arn : ""
}

output "bastion_ssm_private_key_pem_name" {
  value = var.create_bastion_keypair ? module.vpc.bastion_ssm_private_key_pem_name : ""
}

output "s3_artifact_repo_arn" {
  value = module.vpc.s3_artifact_repo_arn
}

output "s3_artifact_repo" {
  value = module.vpc.s3_artifact_repo
}

output "s3_log_bucket" {
  value = module.vpc.s3_log_bucket
}

output "ecr_registry" {
  value = "${data.aws_caller_identity.default.account_id}.dkr.ecr.${var.region}.amazonaws.com"
}

output "tags" {
  value = module.vpc.tags
}
