data "aws_codecommit_repository" "current" {
  repository_name = var.repository_name
}

resource "aws_cloudwatch_event_rule" "codecommit_activity" {
  name_prefix = "${var.branch}-activity"
  description = "Detect commits to CodeCommit repo ${var.repository_name}/${var.branch}"

  event_pattern = <<PATTERN
{
  "source": [ "aws.codecommit" ],
  "detail-type": [ "CodeCommit Repository State Change" ],
  "resources": [ "${data.aws_codecommit_repository.current.arn}" ],
  "detail": {
     "event": [
       "referenceCreated",
       "referenceUpdated"
...
resource "aws_cloudwatch_event_target" "cloudwatch_triggers_pipeline" {
  target_id = "${var.branch}-commits-trigger-pipeline"
...
resource "aws_iam_policy" "cloudwatch_ci_iam_policy" {
  name_prefix = "${var.branch}-cloudwatch-ci-"
