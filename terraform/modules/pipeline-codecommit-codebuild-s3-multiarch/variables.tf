...
variable "image_x86" {
  type = string
  description = "The x86 codebuild runtime image to use"
  default = "aws/codebuild/amazonlinux2-x86_64-standard:2.0"
  #default = "1234567890.dkr.ecr.us-east-1.amazonaws.com/docker-build-base:latest"
}

variable "image_aarch64" {
  type        = string
  description = "The ARM64 codebuild runtime to use for the aarch64 build stage"
...
variable "codebuild_compute_aarch64" {
  type        = string
...
variable "enable_aarch64" {
  type        = bool
  description = "Flag used to toggle aarch64 build support"
  default     = true
}
